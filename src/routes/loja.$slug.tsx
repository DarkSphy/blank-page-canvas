...
function CheckoutModal({ store, cart, total, onClose, onSent }: {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const paymentDisplay = paymentMethod === "cartão" ? "Cartão" : paymentMethod === "dinheiro" ? "Dinheiro" : "Pix";

    // Salvar pedido
    try {
      const finalNotes = notes ? `Pagamento: ${paymentDisplay}\n\n${notes}` : `Pagamento: ${paymentDisplay}`;
      await supabase.from("orders").insert({
        store_id: store.id, full_name: fullName, whatsapp, address,
        notes: finalNotes || null, coupon: coupon || null, delivery_method: delivery,
        items, total,
      });
    } catch {}
            ...
            <div className="flex gap-2">
              {(["pix", "cartão", "dinheiro"] as const).map((p) => (
                <button key={p} type="button" onClick={() => setPaymentMethod(p)}
                  className={`flex-1 h-11 rounded-xl border text-sm font-semibold capitalize ${paymentMethod === p ? "border-accent bg-accent/10 text-accent" : "border-border bg-background"}`}>
                  {p === "cartão" ? "Cartão" : p === "dinheiro" ? "Dinheiro" : "Pix"}
                </button>
              ))}
            </div>
          </div>
